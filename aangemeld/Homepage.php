<?php
session_start();
// print_r( $_SESSION );

if (!isset($_SESSION['ID'])) {
    header("Location:../Homepage.php");
}
?>

<?php include 'header.php';?>
    <section class="homePage">
        <div class="featuredSlider">
            <div class="swiper-container">
                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <img src="images/YLP1.jpg"
                             alt=""/>

                    </div>


                </div>
            </div>
        </div>
        <div class="snippet">
            <p>
                Youth Leadership Development Foundation (YLDF) is a non-governmental and non-profitable organization,
                located in Sana’a, the capital city of Yemen.
            </p>
        </div>
        <div class=" infographics pt-3 pb-3">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">

                        <div class="Links featuredStories site-width">
                            <h2 class="heading_1">Success Stories</h2>
                            <ul class="clearfix">
                                <li class="link clearfix">
                                    <img src="images/Shatha.jpg"
                                         class="icon" alt="Shatha wants to say something"/>
                                         <div><h1>
                    

                    <p class="afbeelding">Shatha wants to say something</p>
                </h1>
                </div>
                                </li>
                                <li class="link clearfix">

                                    <img src="images/emad.jpg"
                                         class="icon" alt="Hard Equation"/>
                                         <div><h2>
                            <p class="education">Hard Equation</p>
                        </h2>
                        
                    </div>
                                </li>
                                <li class="link clearfix">

                                    <img src="images/Wael Success story.jpg"
                                         class="icon" alt="[UIAP] Wael Success Story"/>
                                         <div><h2>
                            <p class="succes_story">[UIAP] Wael Success Story</p>
                        </h2>
                      </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
       

        <div class="videos Links full videosContainer site-width ">
            <div class="row">
                <div class="col-12">
                            <h2 class="heading_1">Videos Displaced Woman (Nazeha) </h2>
                            </div>
                            <div class="col-12">
                <div class="video-overlay active "></div> 
                <div class="cont">
                    <div class="video-container" data-example-id="responsive-embed-16by9-iframe-youtube">
                        <div class="embed-responsive embed-responsive-16by9">
                            <iframe class="embed-responsive-item" id="video"
                                    src="//www.youtube.com/embed/sZgM2jIzkhs?modestbranding=1;controls=1;showinfo=0;rel=0;fs=1"
                                    allowfullscreen=""></iframe>
                        </div>
                    </div>
                </div>
                </div>
            </div>
        </div>
    </section>
    <?php include 'footer.php';?>
   